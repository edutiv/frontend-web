...
const CardSectionDetail = ({ color, title, sidebutton, type, data, refresh, courseId, sectionId }) => {
   ...
   function closeModalUpdateMaterial() {
      setIsOpenUpdateMaterial(false)
   }

   function openModalUpdateMaterial() {
      setIsOpenUpdateMaterial(true)
   }

   const handleSubmitMaterial = (e) => {
      e.preventDefault();
      axios.post(`${BASE_URL}/course/${courseId}/section/${sectionId}/material`, {
   ...
   const handleDetailMaterial = (id) => {
      ...
      .then (function (response) {
         ...
         setMaterialName(response.data.data.material_name);
         setMaterialUrl(response.data.data.material_url);
         openModalUpdateMaterial();
      })
      .catch(function (error) {
         console.log(error);
      })
   ...
   const handleUpdateMaterial = (id) => {
      axios
      .put(`${BASE_URL}/course/${courseId}/section/${sectionId}/material/${id}`, {
         material_type: materialType.name,
         material_name: materialName,
         material_url: materialUrl
      })
      .then(function (response) {
         ...
         setMaterialName("");
         setMaterialUrl("")
         setMaterialType(tipeMaterial[0]);
   ...
   return (
      <>
         ...
         >
                           ...
                           <tr key={items.id}>
                              ...
                              <td className="p-4 px-6 text-xs align-middle border-t-0 border-l-0 border-r-0 whitespace-nowrap">
                                 <div className="flex flex-row gap-3">
                                    <button onClick={() => handleDetailMaterial(items.id)}>
            ...
            <Transition appear show={isOpenUpdateMaterial} as={Fragment}>
               <Dialog as="div" className="relative z-10" onClose={closeModalUpdateMaterial}>
                  <Transition.Child
                     as={Fragment}
                     enter="ease-out duration-300"
                     enterFrom="opacity-0"
                              ...
                              <form className="rounded-b-2xl">
                                 ...
                                 <div className="flex flex-row-reverse w-full gap-3 px-6 py-6 mt-5 bg-slate-200 rounded-b-2xl">
                                    <button
                                       type="button"
                                       className="inline-flex justify-center px-4 py-2 text-sm font-medium text-white border border-transparent rounded-md bg-darkGreen focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                                       onClick={() => handleUpdateMaterial(detailMaterial)}
                                    >
                                       Update Material
                                    </button>
                                    <button
                                       type="button"
                                       className="inline-flex justify-center px-4 py-2 text-sm font-medium text-black border border-transparent border-green-800 rounded-md hover:bg-green-800 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                                       onClick={closeModalUpdateMaterial}
                                    >
                                       Close
                                    </button>
                                 </div>
