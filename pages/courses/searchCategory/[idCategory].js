  ...
  const getEdutivData = () => {
    ...
    Promise.all(endpoints.map((endpoint) => axios.get(endpoint))).then(
      ([{ data: course }, { data: categories }]) => {
        ...
        results = course.data.filter((item) => {
          return item.category.id == idCategory;
          // Use the toLowerCase() method to make it case-insensitive
        });
        setDataFilterCourse(results);
        console.log("resukt", results, idCategory);
  ...
  if (!dataCourse) {
    return <div className="flex flex-col h-screen w-full justify-center items-center">
      <Image 
        ...
        quality={100}
      />
      <p className="text-base text-black">Have a quality time...</p>
    </div>;
  }
