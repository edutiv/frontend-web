...
export default function Learn() {
  ...
  data?.sections.forEach(section => section.materials.forEach(material => {
    totalCourse += 1
  }))
    ...
    <>
      ...
      <div className='mx-8 md:mx-20'>
        <div className='my-10 title flex flex-col md:flex-row justify-between md:items-center text-left'>
          <h4 className='md:text-[28px] text-md'>
            {data?.course_name}
          </h4>
          <button onClick={() => toggleClassShow()} className="flex flex-row gap-1 py-1 px-2 rounded hover:bg-green-700 hover:text-white w-fit self-end md:self-auto">
            <MenuIcon 
          ...
          <Tab.Group vertical selectedIndex={selectedIndex} onChange={setSelectedIndex}>
            ...
            <div className={toggleShow ? 'absolute navbar bg-[#F5F5F5] p-6 rounded-md mb-8 h-fit max-h-[85vh] overflow-auto left-0 top-28 right-0 mr-auto ml-auto z-50 w-[66.6vw]' : 'navbar col-span-4 bg-[#F5F5F5] p-6 rounded-md mb-8 h-fit lg:block hidden'}>
              <Tab.List>
                {
                  toggleShow ? (
                    <button onClick={() => toggleClassShow()} className="relative float-right">
                      <XIcon
                        className='block lg:hidden h-5 w-5'
                      />
                    </button>
                  ) : ''
                }
                      ...
                      <div className='flex flex-col'>
                        {
                          section.materials.map((material) => (
                            <Tab key={material.id} className="focus:outline-none" onClick={() => toggleShow ? toggleClassShow() : ''}>
                              {({ selected }) => (
                                <ButtonLearnNav icon={material.material_type} title={material.material_name} class={selected ? 'true' : ''}/>
                              )}
                            </Tab>
                          ))
                        }
                      ...
                      <Tab.Panel key={material.id}>
                        {
                          material.material_type === "quiz" ? (
                            ...
                            <div className="border-2 rounded-md">
                              <iframe className="w-full h-full rounded-md aspect-video" src={material.material_url} />
                            </div>
                          )
                        }

                        <div className="flex gap-3 md:gap-0 flex-col sm:flex-row md:items-center justify-between py-5">
                          <h4 className="text-md md:text-2xl text-left">{material.material_name}</h4>
                          <div className="flex gap-2 sm:gap-4 flex-col sm:flex-row">
                            <button onClick={() => limitHandlerMin(selectedIndex)} className="bg-white px-5 py-2 text-[#126E64] rounded-md border-[1px] border-[#E0E0E0] text-[11px] hover:border-[#126E64] hover:-translate-y-[0.15rem] hover:transition hover:duration-100 hover:ease-in-out hover:drop-shadow-md focus:outline-none">
                              PREVIOUS
                            </button>
                            <button onClick={() => limitHandlerMax(selectedIndex)} className="px-5 py-2 bg-[#126E64] sm:mr-2 rounded-md text-white text-[11px] hover:bg-[#09423c] hover:-translate-y-[0.15rem] hover:transition hover:duration-100 hover:ease-in-out hover:drop-shadow-md focus:outline-none">
                              NEXT
                            </button>
                          </div>
                        </div>
